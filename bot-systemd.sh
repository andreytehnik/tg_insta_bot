...
# Получаем абсолютную директорию, в которой лежит этот скрипт и проект
...
# Создаем директорию для пользовательских служб, если ее нет
mkdir -p "$SERVICE_DIR"

# Создаем файл службы systemd (без sudo)
...
# Разрешаем фоновым службам работать после закрытия сессии
